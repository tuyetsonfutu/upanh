...
{	
	...
	if($_POST['subform'] == "1")
	{
		...
		if($error == "")
		{			
			$query="UPDATE members SET fullname='".mysql_real_escape_string($fname)."', email='".mysql_real_escape_string($user_email)."', description='".mysql_real_escape_string($details)."', website='".mysql_real_escape_string($website)."', news='".mysql_real_escape_string($news)."', color1='".mysql_real_escape_string($color1)."', mylang='".mysql_real_escape_string($mylang)."' WHERE USERID='".mysql_real_escape_string($SID)."' AND status='1'";
